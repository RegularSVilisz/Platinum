...
Library.ScreenGui = SelfModules.UI.Create("ScreenGui", {
	Name = "SVilisz UI Library",
	ZIndexBehavior = Enum.ZIndexBehavior.Sibling,
})

function Library:Destroy()
	if Library.ScreenGui.Parent then
...
function Library:Notify(options, callback)
	...
	Notification.Frame.Parent = Library.ScreenGui
...
function Library:AddWindow(options)
	...
	Window.Frame = SelfModules.UI.Create("Frame", {
		...
		SelfModules.UI.Create("Frame", {
			...
			SelfModules.UI.Create("ImageLabel", {
				Name = "Glow",
				Image = "http://www.roblox.com/asset/?id=16769099818",
	...
	function Window:Glow(enabled, scale)
		self.Frame.Handler.Glow.Visible = enabled
		self.Frame.Handler.Glow.Size = UDim2.new(1, math.floor(scale), 1, math.floor(scale))
		self.Frame.Handler.Glow.Position = UDim2.new(0, -math.floor(scale/2), 0, -math.floor(scale/2))
	end
	
	function Window:InnerGlow(enabled)
		self.Frame.InnerGlow.Visible = enabled
	end
	
	function Window:Stroke(enabled, thickness)
		self.Frame.Handler.UIStroke.Enabled = enabled
		self.Frame.Handler.UIStroke.Thickness = thickness
	end
	
	function Window:Background(enabled)
		self.Frame.Background.Tabs.Holder.ImageBackGround.Visible = enabled
	end
	...
	local function setAccent(accent)
		...
		Window.Frame.Handler.UIStroke.Color = accent
		Window.Frame.Handler.Glow.ImageColor3 = accent
		Window.Frame.InnerGlow.Left.BackgroundColor3 = accent
		Window.Frame.InnerGlow.Right.BackgroundColor3 = accent
		Window.Frame.InnerGlow.Top.BackgroundColor3 = accent
		Window.Frame.InnerGlow.Bottom.BackgroundColor3 = accent
		...
		for _, tab in next, Window.Tabs do
			for _, section in next, tab.Sections do
				for _, item in next, section.List do
					...
					if item.Type == "DualSlider" then
						item.Frame.Holder.Slider.Bar.Fill.BackgroundColor3 = SelfModules.UI.Color.Sub(accent, Color3.fromRGB(50, 50, 50))
						item.Frame.Holder.Slider.MinPoint.BackgroundColor3 = accent
						item.Frame.Holder.Slider.MaxPoint.BackgroundColor3 = accent
					...
					elseif item.Type == "SubSection" then
						for _, item2 in next, item.List do
							...
							if item2.Type == "DualSlider" then
								item2.Frame.Holder.Slider.Bar.Fill.BackgroundColor3 = SelfModules.UI.Color.Sub(accent, Color3.fromRGB(50, 50, 50))
								item2.Frame.Holder.Slider.MinPoint.BackgroundColor3 = accent
								item2.Frame.Holder.Slider.MaxPoint.BackgroundColor3 = accent
							end
						end
					end
				end
	...
	function Window:AddTab(name, options)
		...
		function Tab:AddSection(name, options)
			...
			function Section:AddDualSlider(name, min, max, defaultMin, defaultMax, options, callback)
				local Slider = {
					Name = name,
					Type = "DualSlider",
					ValueMin = defaultMin,
				...
				local isDraggingMax = false

				-- Functions
				local function getSliderValue(val, isMin)
					val = val or 0 -- If val is nil, set it to 0
					val = math.clamp(val, min or 0, max or 1) -- If min or max are nil, set them to default values
				...
				local function sliderVisual(minVal, maxVal)
					local minPercent = (minVal - min) / (max - min)
					local maxPercent = (maxVal - min) / (max - min)

					Slider.Frame.Holder.Input.Text = minVal .. " | " .. maxVal

					local fillWidth = maxPercent - minPercent
					tween(Slider.Frame.Holder.Slider.Bar.Fill, 0.25, { Size = UDim2.new(fillWidth, 0, 1, 0), Position = UDim2.new(minPercent, 0, 0, 0) })
					tween(Slider.Frame.Holder.Slider.MinPoint, 0.25, { Position = UDim2.new(minPercent, 5, 0.5, 0) })
					tween(Slider.Frame.Holder.Slider.MaxPoint, 0.25, { Position = UDim2.new(maxPercent, -5, 0.5, 0) })
				end

				function Slider:Set(minVal, maxVal)
					minVal = getSliderValue(minVal, true)
					maxVal = getSliderValue(maxVal, false)

					-- Prevent minimum point from clipping over maximum and vice versa
					...
					Slider.ValueMin = minVal
					Slider.ValueMax = maxVal
					sliderVisual(minVal, maxVal)

					pcall(task.spawn, Slider.Callback, minVal, maxVal, Tab.Flags[Slider.Flag] or nil)
				end
				...
				local function startDragging(point, isMin)
					return Mouse.Move:Connect(function()
						local sliderWidth = Slider.Frame.Holder.Slider.AbsoluteSize.X
						local mouseX = Mouse.X
						local absoluteX = Slider.Frame.Holder.Slider.AbsolutePosition.X
						local percent = (mouseX - absoluteX) / sliderWidth

						local range = max - min
						local val = min + percent * range

						if isMin then
							val = math.min(val, Slider.ValueMax - Slider.Gap)
							Slider:Set(val, Slider.ValueMax)
						else
							val = math.max(val, Slider.ValueMin + Slider.Gap)
							Slider:Set(Slider.ValueMin, val)
						end
					end)
				end

				Slider.Frame.Holder.Slider.MinPoint.InputBegan:Connect(function(input)
					if input.UserInputType == Enum.UserInputType.MouseButton1 then
						isDraggingMin = true
						connections.moveMin = startDragging(Slider.Frame.Holder.Slider.MinPoint, true)
					end
				end)

				Slider.Frame.Holder.Slider.MinPoint.InputEnded:Connect(function(input)
					if input.UserInputType == Enum.UserInputType.MouseButton1 then
						isDraggingMin = false
						connections.moveMin:Disconnect()
					end
				end)

				Slider.Frame.Holder.Slider.MaxPoint.InputBegan:Connect(function(input)
					if input.UserInputType == Enum.UserInputType.MouseButton1 then
						isDraggingMax = true
						connections.moveMax = startDragging(Slider.Frame.Holder.Slider.MaxPoint, false)
					end
				end)

				Slider.Frame.Holder.Slider.MaxPoint.InputEnded:Connect(function(input)
					if input.UserInputType == Enum.UserInputType.MouseButton1 then
						isDraggingMax = false
						connections.moveMax:Disconnect()
					end
				end)
				...
				Slider.Frame.Holder.Input.FocusLost:Connect(function()
					...
					local minVal, maxVal = string.match(Slider.Frame.Holder.Input.Text, "(%d+) %- (%d+)")
					if tonumber(minVal) and tonumber(maxVal) then
						Slider:Set(tonumber(minVal), tonumber(maxVal))
					end
				end)

				if options.toggleable == true then
					warn("Toggle is currently not supported for dual slider.")
				end

				Slider:Set(Slider.ValueMin, Slider.ValueMax)
			...
			function Section:AddMultiDropdown(name, list, options, callback)
				...
				function Dropdown:Select(itemName)
					if Dropdown.Selected[itemName] then
						Dropdown.Selected[itemName] = nil
					else
						Dropdown.Selected[itemName] = true
					end

					local selectedItems = {}
					for itemName, isSelected in pairs(Dropdown.Selected) do
						if isSelected then
							table.insert(selectedItems, itemName)
						end
					end

					Dropdown.Frame.Holder.Holder.Displays.Selected.Text = table.concat(selectedItems, ", ")
			...
			function Section:AddSubSection(name, options)
				...
				function SubSection:AddDualSlider(name, min, max, defaultMin, defaultMax, options, callback)
					local Slider = {
						Name = name,
						Type = "DualSlider",
						ValueMin = defaultMin,
